...
class UsersController < ApplicationController
  ...
  def show
    ...
    if r.count > 0
      ...
      @self_comment_bar = (r.first.created_at.to_date..r.last.created_at.to_date).map do |date|
        action = self_comments.count(date.strftime("%b %d"))
        action || 0
      end
      @self_tags_bar = (r.first.created_at.to_date..r.last.created_at.to_date).map do |date|
        action = self_tags.count(date.strftime("%b %d"))
        action || 0
      end

      @other_likes_bar = (r.first.created_at.to_date..r.last.created_at.to_date).map do |date|
        action = other_like.count(date.strftime("%b %d"))
        action || 0
      end
      @other_comment_bar = (r.first.created_at.to_date..r.last.created_at.to_date).map do |date|
        action = other_comment.count(date.strftime("%b %d"))
        action || 0
      end
      @other_bookmarks_bar = (r.first.created_at.to_date..r.last.created_at.to_date).map do |date|
        action = other_bookmark.count(date.strftime("%b %d"))
        action || 0
      end
      @other_tags_bar = (r.first.created_at.to_date..r.last.created_at.to_date).map do |date|
